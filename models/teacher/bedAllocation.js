...
const bedAllocationSelectModel=(req,res)=>{
	...
	where c.classId in (select classId from teacher where teacherId = ${req.session.teacherId||null}) order by s.stuNo;
	...
	join room r  on b.roomId = r.roomId 
	join dormitory d  on d.dormitoryId = da.dormitoryId 
	where c.classId in (select classId from teacher where teacherId = ${req.session.teacherId||null}) ;
	`;
	...
	join room r  on b.roomId = r.roomId 
	join dormitory d  on d.dormitoryId = da.dormitoryId 
	where c.classId in (select classId from teacher where teacherId = ${req.session.teacherId||null})  order by s.stuNo limit ${(getData.pageNo-1)*getData.pageSize},${getData.pageSize};
	select count(*)as count from bedallocation ba
	join student s  on ba.stuNo = s.stuNo 
	join class c  on c.classId = s.classId 
	join college co  on c.collegeId = co.collegeId 
